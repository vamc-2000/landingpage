...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="py-16 bg-gradient-to-b from-white via-orange-50/5 to-white relative overflow-hidden">
        ...
        <div className={`relative rounded-[36px] border border-white/40 bg-white/90 backdrop-blur-md shadow-[0_15px_45px_rgba(0,0,0,0.015)] p-6 lg:p-12 overflow-hidden reveal-on-scroll delay-1 ${isRevealed ? "revealed" : ""}`}>
          ...
          <div className="space-y-6 relative z-10">
            {faqs.map((faq, idx) => (
              <div 
                key={idx} 
                className={`group/card p-7 lg:p-8 rounded-[2rem] border transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] relative overflow-hidden flex flex-col justify-between ${
                  openIndex === idx 
              ...
              >
                ...
                <div className={`absolute top-0 inset-x-0 h-[1.5px] bg-gradient-to-r from-transparent ${
                  openIndex === idx ? "via-primary/20" : "via-black/[0.03]"
                } to-transparent`} />
                ...
                >
                  <div className="pr-6">
                    ...
                    {/* Question Header */}
                    <h3 className={`font-extrabold text-[15.5px] tracking-tight leading-snug transition-colors duration-300 ${
                      openIndex === idx ? "text-primary" : "text-slate-800 group-hover/card:text-slate-900"
                    }`}>
                      {faq.question}
                    </h3>
                  </div>
                  ...
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 shrink-0 ${
                    ...
                    <Plus className={`w-4 h-4 transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                ...
                <div 
                  className={`overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                    openIndex === idx ? "max-h-[300px] opacity-100 mt-6 pt-6 border-t border-slate-100/60" : "max-h-0 opacity-0"
                  }`}
                >
          ...
          {[
            ...
            >
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${
                item.theme === "orange" ? "bg-orange-500/10 text-orange-500" :
